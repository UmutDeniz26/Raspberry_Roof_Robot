...
Servo sg90_vertical, sg90_horizontal;
...
String data, return_output, GPS_data;
...
void setup()
{
    sg90_horizontal.attach(servoHorizontalPin);
    sg90_vertical.attach(servoVerticalPin);
    Serial.begin(19200);
        ...
        if (!error)
        {
            if (!error)
            {
                ...
                else if (type == "camera_move")
                {
                    ...
                    //
                    camera_control(axis, direction, sg90_horizontal.read(), sg90_vertical.read());

                    return_output = "{\"axis\": " + String(axis) + ", \"direction\": " + String(direction) +
                                    ", \"horizontal_position\": " + String(sg90_horizontal.read()) + ", \"vertical_position\": " + String(sg90_vertical.read()) + "}";
                }
                else if (type == "gps")
                {
...
void camera_control(int axis, int direction, int horizontal_position, int vertical_position)
{
    ...
    {
        // Horizontal
        sg90_horizontal.write(horizontal_position);
        delay(500); // Small delay to allow the servo to move
    }
    else if (axis == 1)
    {
        // Vertical
        sg90_vertical.write(vertical_position);
        delay(500); // Small delay to allow the servo to move
    }
}
